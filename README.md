...
Both "Latin1" and "UTF-8" are supersets of "US-ASCII" (plain english alphabet, numbers, punctuation, etc.) so either is perfectly compatible with the ADIF standard, strictly speaking.

But the two encodings are not compatible once you consider "extended characters". More importantly, **"Latin1" assumes that each character is encoded as a single byte**, while **"UTF-8" uses multi-byte sequences** to represent characters outside of "US-ASCII".
...
Add a new header, `ENCODING`, where the generating application can specify any text encoding defined by IANA at https://www.iana.org/assignments/character-sets/character-sets.xhtml
